...
fn compute_upsample(in: ComputeIn) {
    let blur_dimensions: vec2<i32> = vec2<i32>(textureDimensions(u_in_texture).xy);
    ...
    res += 4.0 * textureSampleLevel(u_in_texture, u_in_sampler, uv, 0.0).rgb;
    res += 2.0 * textureSampleLevel(u_in_texture, u_in_sampler, uv + vec2<f32>(r.x, 0.0), 0.0).rgb;
    res += 2.0 * textureSampleLevel(u_in_texture, u_in_sampler, uv + vec2<f32>(-r.x, 0.0), 0.0).rgb;
    res += 2.0 * textureSampleLevel(u_in_texture, u_in_sampler, uv + vec2<f32>(0.0, r.y), 0.0).rgb;
    res += 2.0 * textureSampleLevel(u_in_texture, u_in_sampler, uv + vec2<f32>(0.0, -r.y), 0.0).rgb;
    res += 1.0 * textureSampleLevel(u_in_texture, u_in_sampler, uv + vec2<f32>(r.x, r.y), 0.0).rgb;
    res += 1.0 * textureSampleLevel(u_in_texture, u_in_sampler, uv + vec2<f32>(r.x, -r.y), 0.0).rgb;
    res += 1.0 * textureSampleLevel(u_in_texture, u_in_sampler, uv + vec2<f32>(-r.x, r.y), 0.0).rgb;
    res += 1.0 * textureSampleLevel(u_in_texture, u_in_sampler, uv + vec2<f32>(-r.x, -r.y), 0.0).rgb;
    res /= 16.0;

    textureStore(u_out_texture, out_pos, vec4<f32>(res, 1.0));
}